 ...
 */
 ...
 function TreeSortable() {
     var treeSortable = {
         ...
         createBranch({ id, parent_id, title, level, data }) {
             const {
                 options: {
                     branchSelector,
                     branchPathSelector,
                     ...
                     childrenBusSelector,
                     levelPrefix,
                     dataAttributes: { id: idAttr, parent: parentAttr, level: levelAttr, data: dataAttr },
                 },
                 cleanSelector,
             } = treeSortable;

             let tempPermit = data.permit.length > 0 ? data.permit.join(' & ') : null;
             
             return `
        <li class="${cleanSelector(
             branchSelector
         )} ${levelPrefix}-${level}" data-${idAttr}="${id}" data-${parentAttr}="${parent_id}" data-${levelAttr}="${level}" data-${dataAttr}="${btoa(JSON.stringify(data))}">
            <div class="contents ${!data.enable ? 'text-danger' : ''}">
                    ...
                    <div class="left-sidebar">
                        ...
                        <span class="branch-title">
                            ${data != null ? `
                                ...
                                ${data.permission != null ? `<br><i>Permission : <b>${data.permission}</b></i>` : ``}
                                ${tempPermit ? `<br><small><i>Permit : <b>${tempPermit}</b></i></small>` : ''}
                                ${data.class ? `<br><small><i>Class : <b>${data.class}</b></i></small>` : ''}
                            ` : `${title}`}
                        </span>
                    </div>
                    <div class="right-sidebar">
                        <button type="button" class="button btn-tambah" title="Tambah">
                            <i class="bx bx-plus text-primary"></i>
                        </button>
                        <button type="button" class="button btn-ubah" title="Ubah">
                            <i class="bx bx-pencil text-warning"></i>
                        </button>
                        <button type="button" class="button btn-hapus" title="Hapus">
     ...
     `;
         ...
         jQuerySupplements() {
             ...
             $.fn.extend({
                 ...
                 addChildBranch({id, title, data}) {
                     const {
                         options: {
                             treeSelector,
                             branchSelector,
                             dataAttributes: { id: idAttr },
                             maxLevel,
                         },
                        //  generateUUID,
                         createBranch,
                 ...
                 addSiblingBranch({id, title, data}) {
                     const {
                         options: {
                             treeSelector,
                             branchSelector,
                    ...
                    //  const uid = generateUUID();
                     ...
                     const $element = createBranch({ id, parent_id, level, title, data });
                 ...
                 getBranch() {
                     ...
                     return $(this).closest(`${treeSelector} ${branchSelector}`);
                 },
 
                 updateBranch({id, title, data}) {
                     const {
                         options: {
                            treeSelector,
                            branchSelector,
                            dataAttributes: { parent: parentAttr, level: levelAttr },
                        },
                         createBranch
                     } = treeSortable;
 
                     const $branch = $(this).closest(`${treeSelector} ${branchSelector}`);
                     const parent_id = $branch.data(parentAttr);
                     const level = $branch.data(levelAttr);

                     const $element = createBranch({ id, parent_id, title, level, data });

                     $branch.replaceWith($element);
                 },
             });
         },
